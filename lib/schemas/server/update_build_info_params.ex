defmodule Elidactyl.Schemas.Server.UpdateBuildInfoParams do
  ...
  defp validate_threads(%Changeset{changes: %{threads: threads}} = changeset) when is_binary(threads) do
    threads
    |> String.split(",")
  ...
  defp put_thread_error(changeset) do
    error = "Invalid format. Threads can be a single number, or a comma seperated list. " <>
